...
EditArea.prototype.init = function () {
    ...
    parent.editAreaLoader.add_event(t.result, "click", function (e) {
        if ((e.target || e.srcElement) == editArea.result) {
            editArea.area_select(editArea.textarea.value.length, 0);
        }
    });
...
EditArea.prototype.manage_size = function (onlyOneTime) {
    ...
    if (editAreas[this.id]["displayed"] == true && this.textareaFocused)
    {
        ...
        if (this.last_selection["nb_line"] >= this.line_number)
        {
            ...
            {
                newLines += '<div id="line_' + i + '" onclick="breakPoint(' + i + ');"">' + i + "</div>";
                this.line_number++;
            }
